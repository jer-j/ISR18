...
ADC_HIGH = (
    2**16 - 1
)  # Upper limit of 0 to X scale of digital input from analog to digital convertor
DIGITAL_TO_INCHES = (
    2**10 - 1
) / ADC_HIGH  # Scales to inches based on previous calibration
...
class Sensors:
    ...
    def velocity_calculation(self, dyn_press_read, stat_press_read):
        ...
        velocity_m_per_s = math.sqrt(
            abs((2 / WATER_DENSITY_METRIC) * (dyn_press - stat_press))
        )
    ...
    def read_sensor_vals(self):
        ...
        velocity = self.velocity_calculation(
            dynamic_pressure_reading, static_pressure_reading
        )
        ...
        return (
            ...
            MAGy,
            MAGz,
            dynamic_pressure_reading,
            static_pressure_reading,
            self.yaw,
            self.pitch,
            depth,
            velocity,
