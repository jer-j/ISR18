...
def detectIMU():
    ...
    try:
        ...
        LSM9DS0_WHO_G_response = bus.read_byte_data(
            LSM9DS0_GYR_ADDRESS, LSM9DS0_WHO_AM_I_G
        )
        LSM9DS0_WHO_XM_response = bus.read_byte_data(
    ...
    try:
        ...
        # This section of code stops this from happening.
        LSM9DS1_WHO_XG_response = bus.read_byte_data(
            LSM9DS1_GYR_ADDRESS, LSM9DS1_WHO_AM_I_XG
        )
        LSM9DS1_WHO_M_response = bus.read_byte_data(
    ...
    try:
        ...
        # This section of code stops this from happening.
        LSM6DSL_WHO_AM_I_response = bus.read_byte_data(
            LSM6DSL_ADDRESS, LSM6DSL_WHO_AM_I
        )
        LIS3MDL_WHO_AM_I_response = bus.read_byte_data(
...
def initIMU():
    ...
    if BerryIMUversion == 1:  # For BerryIMUv1
        ...
        writeByte(
            LSM9DS0_ACC_ADDRESS, LSM9DS0_CTRL_REG1_XM, 0b01100111
        )  # z,y,x axis enabled, continuos update,  100Hz data rate
        writeByte(
            LSM9DS0_ACC_ADDRESS, LSM9DS0_CTRL_REG2_XM, 0b00011000
        )  # +/- 8G full scale

        # initialise the magnetometer
        writeByte(
            LSM9DS0_MAG_ADDRESS, LSM9DS0_CTRL_REG5_XM, 0b11110000
        )  # Temp enable, M data rate = 50Hz
        writeByte(LSM9DS0_MAG_ADDRESS, LSM9DS0_CTRL_REG6_XM, 0b01100000)  # +/- 12gauss
        writeByte(
            LSM9DS0_MAG_ADDRESS, LSM9DS0_CTRL_REG7_XM, 0b00000000
        )  # Continuous-conversion mode

        # initialise the gyroscope
        writeByte(
            LSM9DS0_GYR_ADDRESS, LSM9DS0_CTRL_REG1_G, 0b00001111
        )  # Normal power mode, all axes enabled
        writeByte(
            LSM9DS0_GYR_ADDRESS, LSM9DS0_CTRL_REG4_G, 0b00110000
        )  # Continuos update, 2000 dps full scale

    elif BerryIMUversion == 2:  # For BerryIMUv2
        # initialise the accelerometer
        writeByte(
            LSM9DS1_ACC_ADDRESS, LSM9DS1_CTRL_REG5_XL, 0b00111000
        )  # z, y, x axis enabled for accelerometer
        writeByte(LSM9DS1_ACC_ADDRESS, LSM9DS1_CTRL_REG6_XL, 0b00111000)  # +/- 8g

        # initialise the gyroscope
        writeByte(
            LSM9DS1_GYR_ADDRESS, LSM9DS1_CTRL_REG4, 0b00111000
        )  # z, y, x axis enabled for gyro
        writeByte(
            LSM9DS1_GYR_ADDRESS, LSM9DS1_CTRL_REG1_G, 0b10111000
        )  # Gyro ODR = 476Hz, 2000 dps
        writeByte(
            LSM9DS1_GYR_ADDRESS, LSM9DS1_ORIENT_CFG_G, 0b10111000
        )  # Swap orientation

        # initialise the magnetometer
        writeByte(
            LSM9DS1_MAG_ADDRESS, LSM9DS1_CTRL_REG1_M, 0b10011100
        ...
        writeByte(
            LSM9DS1_MAG_ADDRESS, LSM9DS1_CTRL_REG3_M, 0b00000000
        )  # continuos update
        writeByte(
            LSM9DS1_MAG_ADDRESS, LSM9DS1_CTRL_REG4_M, 0b00000000
        )  # lower power mode for Z axis

    elif BerryIMUversion == 3:  # For BerryIMUv3
        # initialise the accelerometer
        writeByte(
            LSM6DSL_ADDRESS, LSM6DSL_CTRL1_XL, 0b10011111
        )  # ODR 3.33 kHz, +/- 8g , BW = 400hz
        writeByte(
            LSM6DSL_ADDRESS, LSM6DSL_CTRL8_XL, 0b11001000
        )  # Low pass filter enabled, BW9, composite filter
        writeByte(
            LSM6DSL_ADDRESS, LSM6DSL_CTRL3_C, 0b01000100
        ...
        # initialise the magnetometer
        writeByte(
            LIS3MDL_ADDRESS, LIS3MDL_CTRL_REG1, 0b11011100
        ...
        writeByte(
            LIS3MDL_ADDRESS, LIS3MDL_CTRL_REG3, 0b00000000
        )  # Continuous-conversion mode
